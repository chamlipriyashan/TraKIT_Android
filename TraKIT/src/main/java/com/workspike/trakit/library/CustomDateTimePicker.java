package com.workspike.trakit.library;

/**
 * Created by Chamli Priyashan on 11/23/2016.
 */
...
public class CustomDateTimePicker implements View.OnClickListener {
    ...
    public void showDialog() {
        if (!dialog.isShowing()) {
            if (calendar_date == null)
                calendar_date = getInstance();

            selectedHour = calendar_date.get(HOUR_OF_DAY);
            selectedMinute = calendar_date.get(MINUTE);

            timePicker.setIs24HourView(false);
            timePicker.setCurrentHour(selectedHour);
            timePicker.setCurrentMinute(selectedMinute);

            datePicker.updateDate(calendar_date.get(YEAR),
                    calendar_date.get(MONTH),
                    calendar_date.get(DATE));
    ...
    public void setTimeIn24HourFormat(int hourIn24Format, int minute) {
        if (hourIn24Format < 24 && hourIn24Format >= 0 && minute >= 0
            ...
            calendar_date.set(calendar_date.get(YEAR),
                    calendar_date.get(MONTH),
                    calendar_date.get(DAY_OF_MONTH), hourIn24Format,
                    minute);
                ...
                if (iCustomDateTimeListener != null) {
                    ...
                    iCustomDateTimeListener.onSet(dialog, calendar_date,
                            calendar_date.getTime(), calendar_date
                                    .get(YEAR),
                            getMonthFullName(calendar_date.get(MONTH)),
                            getMonthShortName(calendar_date.get(MONTH)),
                            calendar_date.get(MONTH), calendar_date
                                    .get(DAY_OF_MONTH),
                            getWeekDayFullName(calendar_date
                                    .get(DAY_OF_WEEK)),
                            getWeekDayShortName(calendar_date
                                    .get(DAY_OF_WEEK)), calendar_date
                                    .get(HOUR_OF_DAY),
                            getHourIn12Format(calendar_date
                                    .get(HOUR_OF_DAY)), calendar_date
                                    .get(MINUTE), calendar_date
                                    .get(SECOND), getAMPM(calendar_date));
                }
    ...
    private String getMonthFullName(int monthNumber) {
        Calendar calendar = getInstance();
        calendar.set(MONTH, monthNumber);
    ...
    private String getWeekDayFullName(int weekDayNumber) {
        Calendar calendar = getInstance();
        calendar.set(DAY_OF_WEEK, weekDayNumber);
    ...
    private String getAMPM(Calendar calendar) {
        String ampm = (calendar.get(AM_PM) == Calendar.AM) ? "AM"
                : "PM";
        return ampm;
    }
